  ...
  return (
    ...
    >
      ...
      <Box m={1} sx={{ width: "100%" }}>
        ...
        {/* <math-field read-only>{q.question_data.title}</math-field> */}
        ...
        {q.question_data.type === "a" && (
          <FormControl>
            <RadioGroup value={q.option_data.find((opt) => opt.correct).option}>
              {q.option_data.map((l) => (
                <FormControlLabel
                  value={l.option}
                  control={<Radio />}
        ...
        <Dialog open={openDownvoteReason} onClose={handleCloseDownvoteReason}>
          ...
          <DialogContent>
            <DialogContentText>{multi.downvoteModalSubHead}</DialogContentText>
            <Box pt={2}>
              <TextField
                autoFocus
                margin="dense"
